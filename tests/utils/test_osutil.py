...
from utils.osutil import OS, get_os, get_app_data_dir
...
class TestGetAppDataDir(unittest.TestCase):
    def test_creates_directory(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("utils.osutil.get_os", return_value=OS.LINUX):
    ...
    def test_creates_missing_parent_directories(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("utils.osutil.get_os", return_value=OS.LINUX):
                with patch.dict(os.environ, {"XDG_DATA_HOME": str(xdg_path)}):
                    data_dir = get_app_data_dir()
    ...
    def test_windows_path(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("utils.osutil.get_os", return_value=OS.WINDOWS):
    ...
    def test_macos_path(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("utils.osutil.get_os", return_value=OS.MACOS):
